Shader "Hidden/SgtAtmosphereOuter"
{
	Properties
	{
		...
		_ScatteringScale("Scattering Scale", Float) = 0.5
	...
	SubShader
	{
		...
		Pass
		{
			...
			void Frag(v2f i, out f2g o)
			{
				...
				i.texcoord0.y = dot(i.texcoord4, normalize(i.texcoord1.xyz));
				//i.texcoord0.y *= pow(abs(i.texcoord0.y), _ScatteringScale);
				i.texcoord0.y = i.texcoord0.y * 0.5f + 0.5f;

				float4 scattering = tex2D(_ScatteringTex, i.texcoord0) * _Light1Scatter;
			#if LIGHT_2
				i.texcoord0.y = dot(i.texcoord4, normalize(i.texcoord2.xyz));
				//i.texcoord0.y *= pow(abs(i.texcoord0.y), _ScatteringScale);
				i.texcoord0.y = i.texcoord0.y * 0.5f + 0.5f;
